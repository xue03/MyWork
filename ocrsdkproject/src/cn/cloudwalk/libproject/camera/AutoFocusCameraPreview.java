...
public class AutoFocusCameraPreview extends SurfaceView implements SurfaceHolder.Callback,
    ...
    private int width;
    private int height;
    private int ocrRectW;//ocr扫描矩形框宽
    private int ocrRectH;//ocr扫描矩形框高
	...
	*/
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int previewWidth = mCameraConfigurationManager.getCameraResolution().x;//获取Camera预览尺寸宽
        int previewHeight = mCameraConfigurationManager.getCameraResolution().y;//获取Camera预览尺寸高
        int screenWidth = getMeasuredWidth();
        int screenHeight = getMeasuredHeight();
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            int realOcrWidth = (int)(1.0f*previewWidth * 0.7);
            int realOcrHeight = (int)(1.0f*realOcrWidth/ratio);
            ocrRectW = (int)(1.0f*screenWidth/(1.0f*previewWidth)*realOcrWidth);
            ocrRectH = (int)(1.0f*screenHeight/(1.0f*previewHeight)*realOcrHeight);
        } else {

            //竖屏识别横板卡片
            if (((Activity) getContext()).getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
                int realOcrWidth = (int) (1.0f * previewHeight * 0.9);//卡片实际占宽
                int realOcrHeight = (int) (1.0f * realOcrWidth / ratio);//卡片实际占宽
                ocrRectW = (int) (1.0f * screenWidth * 0.9);//矩形框宽
                ocrRectH = (int) (1.0f * realOcrHeight * screenHeight / previewWidth);
            }

            //竖屏识别竖版银行卡
            if (Contants.OCR_FLAG_BANKCARD == mFlag && mAutoRatio) {//银行卡
                //竖屏的模板框，宽<高
                int realOcrWidth = (int)(1.0f*previewHeight*0.7);
                int realOcrHeight = (int)(1.0f*realOcrWidth*ratio);
                ocrRectW = (int)(1.0f*screenWidth/(1.0f*previewHeight)*realOcrWidth);
                ocrRectH = (int)(1.0f*screenHeight/(1.0f*previewWidth)*realOcrHeight);
            }
        }
        ...
        if (isFocusedOk) {
            ...
            if (Contants.OCR_FLAG_BANKCARD == mFlag)
                ...
                IDCardSDK.getInstance(context).cwPushFrame(data, w, h
                        , BankOcrInterface.CW_BankOcrFormat.CW_COLORTYPE_NV21, mFlag, left_topx, left_topy, right_bottomx, right_bottomy,getResources().getConfiguration().orientation);
            }
        }
    }
